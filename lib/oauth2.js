...
// it will redirect the user to the original URL they requested
...
// [START middleware]
// Middleware that requires the user to be a client. If the user is not a client,
// it will redirect the user to the original URL they requested
function clientRequired (req, res, next) {
  var clientName = null;
  getModel().verifyAdmin(req.user.id, 2, (err, savedData) => {
    if (err) {
      next(err);
      return;
    }
    if(typeof savedData[0] != 'undefined') {
      clientName = savedData[0].profileName;
      next();
    }
    if (!clientName) {
      req.session.oauth2return = req.originalUrl;
...
router.get(
  ...
  (req, res) => {
    const profileData = {profileID:"", profileName:""};
    profileData.profileID = req.user.id;
    profileData.profileName = req.user.displayName;
    // Try to create a user in the database if it is their first time logging in
    getModel().createUser(profileData, req.user.displayName, (err, savedData) => {
      if (err) {
        next(err);
        return;
      }
    });
    // Set a user as active after authentication is done
